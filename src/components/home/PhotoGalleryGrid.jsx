...
export default function PhotoGalleryGrid() {
  ...
  return (
    <section className="py-16 bg-muted/30" aria-label="גלריית פעילות הפורום">
        ...
        <div className="columns-2 sm:columns-3 lg:columns-4 gap-3 space-y-3">
          {allPhotos.map((photo, i) => (
            <motion.div
              ...
              className="break-inside-avoid cursor-pointer rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 group"
      ...
      <AnimatePresence>
        {lightbox && (
          ...
          >
            <button
              className="absolute top-4 left-4 text-white hover:text-secondary"
              onClick={() => setLightbox(null)}
            >
              <X className="w-8 h-8" />
            </button>
